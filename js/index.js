import { InputValedate, getdata, createRow } from "./function.js";
...
button && button.addEventListener('click', function (e) {
    e.preventDefault();
    let isValid = InputValedate(input)
    if (isValid) {
        const todo = {
            name: input.value,
            status: 'todo',
    ...
    if (deleteButton.length) {
        deleteButton.forEach((del) => {
            del && del.addEventListener('click', function () {
                let isDelete = confirm('Rostdan ham ushbu malumotni ochirib tashlamoqchimisiz?')
